// preload.ts — Runs in an isolated context with access to Node/Electron APIs.
...
//
// The object below is written `satisfies ElectronAPI` (@shared/types) so any
// drift between this file and the shared contract — a renamed method, a
...
const electronAPI = {
  ...
  /** Subscribes to update-check/download/install progress. */
   ...
   * execFile+wait) — see shell/process-manager.ts. Returns {ok, error?}
  ...
  /** Subscribes to background-process output chunks. */
  onProcessOutput: (callback) => {
    const listener = (_event: unknown, data: ProcessOutputEvent) => callback(data);
    ipcRenderer.on('process-output', listener);
    return () => ipcRenderer.removeListener('process-output', listener);
  },

  /** Subscribes to background-process lifecycle changes. */
  onProcessStatus: (callback) => {
    const listener = (_event: unknown, data: ProcessStatusEvent) => callback(data);
