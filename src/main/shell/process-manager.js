// shell/process-manager.js — long-running "background process" snippets:
// dev servers, `docker compose up`, `tail -f`, watchers. Unlike exec.js's
...
// user explicitly stops it or the whole app quits. Reuses exec.js's
...
// (see stopAll(), wired into index.js's quit handlers), and there's no
// "was running before a crash" recovery. That's a deliberate scope
// boundary, not an oversight.
'use strict';

const { spawn, execFile } = require('node:child_process');
const { buildInvocation } = require('./exec');
...
/** @type {Map<string, object>} snippetId -> record */
...
function clearTimers(record) {
  ...
  if (record.healthyTimer) { clearTimeout(record.healthyTimer); record.healthyTimer = null; }
}
...
function spawnChild(record) {
  ...
  const tryNext = () => {
    if (i >= candidates.length) {
      record.status = 'error';
      send('process-status', { snippetId, status: 'error', message: `Could not find a "${shellType}" executable. Tried: ${candidates.join(', ')}` });
      return;
    }
    const file = candidates[i++];
    let child;
    try {
      child = spawn(file, args, { cwd: cwd || undefined, env, windowsHide: true });
    } catch (err) {
      record.status = 'error';
      send('process-status', { snippetId, status: 'error', message: String(err.message || err) });
      return;
    }
    record.child = child;
    ...
    child.on('spawn', () => {
      ...
      record.healthyTimer = setTimeout(() => { record.restartCount = 0; }, HEALTHY_RESET_MS);
    });
    child.stdout.on('data', (chunk) => send('process-output', { snippetId, stream: 'stdout', chunk: chunk.toString('utf8') }));
    child.stderr.on('data', (chunk) => send('process-output', { snippetId, stream: 'stderr', chunk: chunk.toString('utf8') }));
    child.on('error', (err) => {
      if (!startedOk && err.code === 'ENOENT' && i < candidates.length) { tryNext(); return; }
      record.status = 'error';
      send('process-status', { snippetId, status: 'error', message: String(err.message || err) });
    });
    child.on('exit', (code, signal) => {
      ...
      if (!manualStop && record.status === 'crashed' && record.options.autoRestart) {
        if (record.restartCount < MAX_AUTO_RESTARTS) {
          ...
          send('process-status', { snippetId, status: 'restart-limit', message: `Gave up after ${MAX_AUTO_RESTARTS} automatic restarts.` });
        }
      }
    });
  };
 ...
 */
function startProcess({ snippetId, command, cwd, shellType, env, autoRestart }) {
  ...
  const record = {
    ...
    healthyTimer: null,
    startedAt: null,
    options: { command, cwd, shellType, env, autoRestart: Boolean(autoRestart) },
  };
...
async function stopProcess(snippetId) {
...
async function restartProcess(snippetId) {
  const record = processes.get(snippetId);
...
/** Stops every live process — called on app quit (see index.js) so nothing is orphaned when the app actually exits (as opposed to just being hidden, which leaves them running on purpose). */
function stopAll() {
  return Promise.all(Array.from(processes.keys()).map((id) => stopProcess(id)));
}

module.exports = { startProcess, stopProcess, restartProcess, listProcesses, stopAll };
