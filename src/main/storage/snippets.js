// storage/snippets.js — the snippet library: schema, defaults, read/write.
...
const { SNIPPETS_FILE } = require('../paths');
const { newId } = require('../id');
const { backupSnippetsIfDue } = require('./backups');
const { readJsonFileSafe } = require('../json-file');

const VALID_SHELLS = ['powershell', 'cmd', 'gitbash', 'wsl', 'node', 'python'];

const DEFAULT_SNIPPETS = [
  // --- git -----------------------------------------------------------------
  { id: 'git-status', name: 'Git status', tag: 'git',
    command: 'git status' },
  { id: 'git-log-recent', name: 'Recent commits', tag: 'git',
    command: 'git log --oneline -10' },
  { id: 'git-current-branch', name: 'Current branch', tag: 'git',
    command: 'git branch --show-current' },
  { id: 'git-pull', name: 'Pull latest', tag: 'git',
    command: 'git pull' },
  { id: 'git-diff', name: 'Uncommitted changes', tag: 'git',
    command: 'git diff' },
  { id: 'git-branches', name: 'List all branches', tag: 'git',
    command: 'git branch -a' },

  // --- npm -------------------------------------------------------------------
  { id: 'npm-install', name: 'Install dependencies', tag: 'npm',
    command: 'npm install' },
  { id: 'npm-run-dev', name: 'Run dev server', tag: 'npm',
    command: 'npm run dev' },
  { id: 'npm-run-build', name: 'Build', tag: 'npm',
    command: 'npm run build' },
  { id: 'npm-outdated', name: 'Outdated packages', tag: 'npm',
    command: 'npm outdated' },
  { id: 'npm-global-list', name: 'Global packages', tag: 'npm',
    command: 'npm list -g --depth=0' },
  { id: 'npm-cache-clean', name: 'Clear npm cache', tag: 'npm',
    command: 'npm cache clean --force' },

  // --- docker ------------------------------------------------------------------
  { id: 'docker-ps', name: 'Running containers', tag: 'docker',
    command: 'docker ps' },
  { id: 'docker-ps-all', name: 'All containers', tag: 'docker',
    command: 'docker ps -a' },
  { id: 'docker-images', name: 'List images', tag: 'docker',
    command: 'docker images' },
  { id: 'docker-compose-up', name: 'Compose up (detached)', tag: 'docker',
    command: 'docker compose up -d' },
  { id: 'docker-compose-down', name: 'Compose down', tag: 'docker',
    command: 'docker compose down' },
  { id: 'docker-logs', name: 'Follow container logs', tag: 'docker',
    command: 'docker logs -f {{container}}' },
  { id: 'docker-prune', name: 'Clean up unused data', tag: 'docker',
  ...
  { id: 'ping-host', name: 'Ping a host', tag: 'network',
  ...
  { id: 'ip-config', name: 'IP configuration', tag: 'network',
    command: 'ipconfig /all' },
  { id: 'network-status', name: 'Network status (google.com)', tag: 'network',
  ...
  { id: 'top-memory-processes', name: 'Top 5 memory processes', tag: 'system',
    command: 'Get-Process | Sort-Object WS -Descending | Select-Object -First 5 Name,@{N="MemoryMB";E={[math]::Round($_.WS/1MB,1)}}' },
  { id: 'system-uptime', name: 'System uptime', tag: 'system',
    command: '(Get-Date) - (Get-CimInstance Win32_OperatingSystem).LastBootUpTime' },
  { id: 'kill-process-by-name', name: 'Kill process by name', tag: 'system',
  ...
  { id: 'env-variables', name: 'Environment variables', tag: 'system',
  ...
  { id: 'temp-folder-size', name: 'Measure temp folder size', tag: 'files',
    command: 'Get-ChildItem $env:TEMP -Recurse -File -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum | Select-Object @{N="SizeMB";E={[math]::Round($_.Sum/1MB,1)}},Count' },
  ...
  // --- utility ------------------------------------------------------------------
  { id: 'current-user', name: 'Current user & groups', tag: 'utility',
    command: 'whoami; whoami /groups' },
  { id: 'installed-apps', name: 'Installed applications', tag: 'utility',
    command: "Get-ItemProperty 'HKLM:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\*' | Select-Object DisplayName,DisplayVersion | Where-Object DisplayName | Sort-Object DisplayName" },
  { id: 'open-url', name: 'Open a URL', tag: 'utility',
...
function sanitizeExpect(exp) {
  if (!exp || typeof exp !== 'object') return null;
  const exitCode = Number.isFinite(exp.exitCode) ? exp.exitCode : null;
  const outputContains = typeof exp.outputContains === 'string' && exp.outputContains.trim()
    ? exp.outputContains.slice(0, 500)
    : null;
  if (exitCode === null && !outputContains) return null;
  return { exitCode, outputContains };
}

function sanitizeSchedule(sch) {
  if (!sch || typeof sch !== 'object') return null;
  const type = ['interval', 'daily', 'cron'].includes(sch.type) ? sch.type : 'interval';
  return {
    enabled: Boolean(sch.enabled),
    type,
    intervalMinutes: Number.isFinite(sch.intervalMinutes) ? Math.max(1, Math.round(sch.intervalMinutes)) : 60,
    dailyTime: /^\d{2}:\d{2}$/.test(sch.dailyTime || '') ? sch.dailyTime : '09:00',
    cronExpr: typeof sch.cronExpr === 'string' && sch.cronExpr.trim() ? sch.cronExpr.trim().slice(0, 100) : '*/15 * * * *',
    lastRunAt: sch.lastRunAt ? String(sch.lastRunAt) : null,
  };
}
...
function sanitizeSnippet(s) {
  const steps = Array.isArray(s.steps)
    ? s.steps.map((step) => String(step).slice(0, 5000)).slice(0, 20).filter(Boolean)
    : null;
  return {
    ...
    runCount: Number.isFinite(s.runCount) ? s.runCount : 0,
    lastRunAt: s.lastRunAt ? String(s.lastRunAt) : null,
    cwd: s.cwd ? String(s.cwd).slice(0, 1000) : null,
    shell: VALID_SHELLS.includes(s.shell) ? s.shell : 'powershell',
...
function readSnippets() {
  ensureSnippetsFile();
  const parsed = readJsonFileSafe(SNIPPETS_FILE, DEFAULT_SNIPPETS, Array.isArray);
...
function writeSnippets(snippets) {
  ...
  backupSnippetsIfDue(); // snapshot the pre-change state (throttled, see storage/backups.js)
  fs.mkdirSync(path.dirname(SNIPPETS_FILE), { recursive: true });
  fs.writeFileSync(SNIPPETS_FILE, JSON.stringify(sanitized, null, 2), 'utf8');
  return sanitized;
}

module.exports = { ensureSnippetsFile, readSnippets, writeSnippets, sanitizeSnippet, VALID_SHELLS };
